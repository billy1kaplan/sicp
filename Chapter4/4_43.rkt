...
(define program '(begin
                   ...
                   (define xor (lambda (val1 val2)
                                 (and (or val1 val2)
                                      (not (and val1 val2)))))
                   ;; Arbitrary order of the yacht owners: Mary Moore's Father, Colonel Downing, Mr. Hall, Sir Barnacle Hood, Dr. Parker
                   ;; Daughters: MaryAnn, Gabrielle, Lorna, Melissa, Rosalind
                   (define assoc (lambda (key items)
                                   (cond ((null? items) '())
                                         ((eq? (car (car items)) key) (car (cdr (car items))))
                                         (else (assoc key (cdr items))))))
                   (define yacht-puzzle (lambda ()
                                          ;; Yacht owner's don't name boats after their own daughter's (allegedly)
                                          (let ((maryann (amb 'moore 'downing 'hall 'hood 'parker))
                                            ...
                                            (let ((mappings (list (list 'hood gabrielle)
                                                                 (list 'moore lorna)
                                                                 (list 'hall rosalind)
                                            ...
                                            (begin
                                              ...
                                              (require (eq? (assoc gabrielle mappings) 'parker))
                                              (require (distinct? (list maryann gabrielle lorna melissa rosalind)))
                                              (display (list (list 'MaryAnn maryann)
...
;; For the orignal problem, here is the solution:
;; ((MaryAnn moore) (Gabrielle hall) (Lorna downing) (Melissa hood) (Rosalind parker))

;; Without knowing Mary Ann's last name, the solutions are:
;; ((MaryAnn moore) (Gabrielle hall) (Lorna downing) (Melissa hood) (Rosalind parker))
;; ((MaryAnn hall) (Gabrielle moore) (Lorna parker) (Melissa hood) (Rosalind downing))

;; Improving speed is a matter of solving part of the problem ourselves!
;; Expensive constraints will be worked on last, failures will occur and be prefiltered (by our minds)

(define fast-program '(begin
                   (define require (lambda (p)
                                     (if (not p)
                                         (amb)
                                         'ok)))
