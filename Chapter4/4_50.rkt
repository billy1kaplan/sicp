...
(define (analyze-ramb exp)
  (define (nth n list)
    (if (= n 0)
        (car list)
        (nth (- n 1) (cdr list))))
  (define (remove n list)
    (if (= n 0)
        (cdr list)
        (cons (car list)
              (remove (- n 1) (cdr list)))))
  (define (scramble acc choices)
    (if (null? choices)
        acc
        (let ((choice (random 0 (length choices))))
          (scramble (cons (nth choice choices) acc)
                    (remove choice choices)))))
  (analyze-amb (cons 'amb
                     (scramble '() (amb-choices exp)))))
...
(define program '(begin
                   ...
                   (parse '(the big professor lectures))))
;; The green class lsleeps (uses the phrase as template).
