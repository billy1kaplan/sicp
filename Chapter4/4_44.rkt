...
(define program '(begin
                   ...
                   (define queens-iter (lambda (board i queens)
                                         (if (>= i queens)
                                             ;; To display all of the solutions; once we find one, print it, then fail and find the others
                                             (begin (display board) (newline) (amb))
                                             (let ((new-col (an-integer-between 0 queens)))
                                                   (let ((new-board (cons new-col board)))
                                                    (begin
                                                      (require (safe? (+ i 1) new-board))
                                                      (queens-iter new-board (+ i 1) queens)))))))
                   (queens-iter '() 0 5)))

;; This appears to work (n = 5 => 10 solutions)
;; Could fix:
;; Add special if-syntax to remove "begin" from require
;; Allow special define syntax to avoid using lambda's
;; Allow inner defines
