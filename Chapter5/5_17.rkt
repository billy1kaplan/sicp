      ...
      (define (execute)
        (let ((insts (get-contents pc)))
          (if (null? insts)
              'done
              (begin
                (if (not (null? (instruction-label (mcar insts))))
                    (begin (display (instruction-label (mcar insts)))
                           (newline))
                    (void))
                (display (instruction-text (mcar insts)))
                (newline)
...
(define (extract-labels text preceeding-label receive)
  (if (null? text)
      (receive '() '())
                          ...
                          (if (symbol? next-inst)
                              (receive insts
                                       (mcons (make-label-entry next-inst
                                                                insts)
                                             labels))
                              (receive (mcons (make-instruction next-inst preceeding-label)
                                              insts)
                                       labels)))))))
...
(define (make-instruction label text)
  (mcons (mcons text label) '()))

(define (mcdar exp) (mcdr (mcar exp)))
...
(define (mcaddr inst) (mcadr (mcdr inst)))
...
(set-register-contents! fib-rec 'n 3)
(start fib-rec)
